...
# 06. Compare proteins and p-tau217
# Requires: Scripts 04–05 outputs
# Produces: nested model comparisons and private OOF predictions
...
# =====================================================
# COMMON COHORT
# proteins + pTau217 only
# =====================================================

N0=len(df)
COMMON_COLS=protein_cols+["p.tau217"]
...
cohort_export = df[[TARGET, "p.tau217"]].reset_index()
...
oof_ptau = pd.Series(np.nan,index=df.index.astype(str))
...
for fold in range(1,6):
    ...
    X_train = X_ptau.loc[train_ids].copy()
    X_test  = X_ptau.loc[test_ids].copy()
    scaler=StandardScaler()
    X_train=scaler.fit_transform(X_train)
    X_test=scaler.transform(X_test)
    ...
    search.fit(X_train,y_outer_train)
    y_prob=search.best_estimator_.decision_function(X_test)
    oof_ptau.loc[test_ids]=(y_prob-y_prob.mean())/(y_prob.std()+1e-8)
    y_pred=(y_prob>=0).astype(int)
    tn,fp,fn,tp=confusion_matrix(y_outer_test,y_pred).ravel()
    auc=roc_auc_score(y_outer_test,y_prob)
    best_params_ptau.append({
        "Fold": fold,
        "kernel": search.best_params_["kernel"],
        "C": search.best_params_["C"],
        "gamma": search.best_params_["gamma"],
    ...
    fpr,tpr,thr=roc_curve(y_outer_test,y_prob)
    roc_store.append(pd.DataFrame({
    "Fold":fold,"Model":"p-tau217","FPR":fpr,"TPR":tpr,"Threshold":thr,"AUC":auc}))
    ...
    print(f"pTau AUC={metrics_ptau[-1]['AUC']:.3f}")
    ...
    X_ptau.loc[train_ids].reset_index(drop=True)
    ],axis=1)

    X_test_combo=pd.concat([
    X_outer_test[panel].reset_index(drop=True),
    X_ptau.loc[test_ids].reset_index(drop=True)
    ],axis=1)
    ...
    fpr,tpr,thr=roc_curve(y_outer_test,y_prob)
    roc_store.append(pd.DataFrame({
    "Fold":fold,"Model":"Proteins + p-tau217","FPR":fpr,"TPR":tpr,"Threshold":thr,"AUC":auc}))
...
pd.DataFrame(best_params_ptau).to_csv(RESULTS_DIR/"best_params_outer_ptau.csv",index=False)
...
metrics_ptau.mean(numeric_only=True).to_csv(RESULTS_DIR/"metrics_mean_ptau.csv")
...
"Model":["SomaScan","pTau217","Combo"],
"Mean_CN":[oof_soma[y==0].mean(),oof_ptau[y==0].mean(),oof_combo[y==0].mean()],
"Mean_AD":[oof_soma[y==1].mean(),oof_ptau[y==1].mean(),oof_combo[y==1].mean()]
})
...
for name,a,b in [
("Soma_vs_pTau",oof_soma,oof_ptau),
...
for tag, panel_final in final_panels.items():
    ...
    X_ptau.reset_index(drop=True)],axis=1)
    scaler=StandardScaler()
