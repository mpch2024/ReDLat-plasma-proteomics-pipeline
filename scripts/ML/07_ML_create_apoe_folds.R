...
# 07. Create APOE nested folds
# Requires: private master matrix
# Produces: five stratified outer folds for the APOE subset
...
OUT_DIR <- file.path(ML_CONFIG$private_root, "apoe", "00_folds")
...
cat("\nN ApoE missing after filter:",
    sum(is.na(meta$ApoE)),
    "\n")

cat("Subjects after filter:",
    nrow(meta),
    "\n")
