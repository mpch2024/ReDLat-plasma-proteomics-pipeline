...
# 19. Generate fixed-panel p-tau217 panels
...
RESULTS_DIR = CONFIG.publication_root / "figures" / "figure_4" / "ptau"
RESULTS_DIR.mkdir(parents=True, exist_ok=True)
ROC_FILE = CONFIG.private_root / "fixed_panel_ptau" / "roc_fold_curves.csv"
AUC_FILE = CONFIG.private_root / "fixed_panel_ptau" / "auc_summary.csv"
R2_FILE = CONFIG.private_root / "fixed_panel_ptau" / "model_statistics.csv"
...
def plot_roc():
    ...
    "Panel":"#0057D9",
    "pTau217":"#D94B00",
    "Panel+pTau217":"#009E73"
    }

    auc_lookup = dict(zip(auc_df["Model"],auc_df["AUC"]))

    text=[]

    mean_fpr=np.linspace(0,1,100)

    for model in ["Panel","pTau217","Panel+pTau217"]:
        tmp_model=roc[
        roc["Model"]==model
        ]
        tprs=[]
        ...
        text.append((model,colors[model],f"{auc_lookup[model]:.3f}"))
...
def plot_r2():
    ...
    vals = r2_df["McFadden_R2"]

    ax.bar(
        r2_df["Model"],
        vals,
        color=[colors[m] for m in r2_df["Model"]],
        width=0.7
    )
    ...
    ax.set_ylabel("McFadden R²",fontsize=7)
    ax.set_ylim(0,0.45)
    ...
    plt.savefig(RESULTS_DIR/"McFadden_R2.pdf",dpi=600,pad_inches=0)
