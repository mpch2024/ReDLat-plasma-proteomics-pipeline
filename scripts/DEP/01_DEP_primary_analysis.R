...
###############################################################################
# 00. Configuration and dependencies
###############################################################################


# Inputs are supplied through config/dep_config.R or environment variables.
data_dir <- DEP_CONFIG$data_dir
csv_file <- DEP_CONFIG$metadata_file
adat_file <- DEP_CONFIG$adat_file
outdir <- project_root
...
RUN_FLAGS <- list(
  ...
  run_country_loco = FALSE,
  run_country_meta = FALSE,
  run_country_interaction = FALSE,
  run_site_robustness = FALSE,
  run_balanced_country_resampling = FALSE,
  run_formal_robustness_classification = FALSE
)
...
if (length(missing_pkgs) > 0) {
  stop(
    ...
    "\nRun renv::restore() before running this pipeline."
  )
}
...
subdirs <- c(
  "00_logs",
  "01_qc", "01_qc/sample_tracking", "01_qc/annotations",
  "02_pca",
  "03_dep", "03_dep/aptamer_level", "03_dep/gene_collapsed",
  "04_sensitivity", "04_sensitivity/apoe", "04_sensitivity/cdrsb",
  "04_sensitivity/cdrsb/AD_only", "04_sensitivity/cdrsb/AD_vs_CN_adjusted",
  "04_sensitivity/vascular_metabolic", "04_sensitivity/atn_adjusted", "04_sensitivity/clinical_severity",
  "05_enrichment_corrected", "05_enrichment_corrected/gsea", "05_enrichment_corrected/ora",
  "06_robustness", "06_robustness/country_loco/tables",
  "06_robustness/country_meta/tables", "06_robustness/country_interaction/tables",
  "06_robustness/site_robustness", "06_robustness/site_robustness/loso/tables",
  "06_robustness/balanced_country_resampling/tables",
  "06_robustness/formal_classification",
  "07_manifest", "07_manifest/model_diagnostics", "workspace",
  "final_source_data"
)
...
safe_write_csv(tibble::tibble(AptName = protein_universe), file.path(analysis_root, "01_qc", "sample_tracking", "protein_universe_internal_adat.csv"))
safe_write_csv(protein_universe_audit, file.path(analysis_root, "01_qc", "sample_tracking", "protein_universe_audit.csv"))
...
safe_write_csv(sample_tracking_tbl, file.path(analysis_root, "01_qc", "sample_tracking", "sample_tracking_workflow.csv"))
...
safe_write_csv(summary_tbl, file.path(analysis_root, "01_qc", "Table_1_sample_characteristics_by_group.csv"))
safe_write_csv(na_count_tbl, file.path(analysis_root, "01_qc", "metadata_missingness_count.csv"))
...
safe_write_csv(normalized_expr_all, file.path(analysis_root, "private", "pca", "normalized_log2_z_expression_all_samples.csv"))
safe_write_csv(normalized_expr_CN_AD, file.path(analysis_root, "private", "pca", "normalized_log2_z_expression_CN_AD.csv"))
...
if (RUN_FLAGS$run_pca) {
  ...
  safe_write_csv(pca_var, file.path(analysis_root, "02_pca", "pca_variance_explained.csv"))
  safe_write_csv(pca_df, file.path(analysis_root, "private", "pca", "pca_scores_with_metadata.csv"))
  pca_df_public <- pca_df %>% dplyr::select(-dplyr::any_of(c("SampleId", "SiteId", "site", "PlateId"))) %>% dplyr::mutate(Record = sprintf("P%04d", dplyr::row_number()), .before = 1)
  safe_write_csv(pca_df_public, file.path(analysis_root, "02_pca", "pca_scores_privacy_safe.csv"))
}
...
safe_write_csv(tibble::tibble(SampleId = dep_df$SampleId), file.path(analysis_root, "private", "sample_tracking", "main_dep_sample_ids.csv"))
...
export_dep_table(DEP_aptamer, file.path(analysis_root, "03_dep", "aptamer_level", "AD_vs_CN_full_limma_results_aptamer_level.csv"))
export_dep_table(DEP_gene, file.path(analysis_root, "03_dep", "gene_collapsed", "AD_vs_CN_full_limma_results_gene_collapsed.csv"))
safe_write_csv(DEP_counts, file.path(analysis_root, "03_dep", "AD_vs_CN_DEP_counts_by_universe.csv"))

# Main DEP filtered tables for direct FDR 0.05 / FDR 0.01 reporting.
export_fdr_specific_dep_tables(
  DEP_gene,
  file.path(analysis_root, "03_dep", "gene_collapsed", "FDR_specific"),
  "AD_vs_CN_DEP_gene_collapsed"
)
export_fdr_specific_dep_tables(
  DEP_aptamer,
  file.path(analysis_root, "03_dep", "aptamer_level", "FDR_specific"),
...
if (RUN_FLAGS$run_apoe_sensitivity) {
  ...
  if (nrow(dep_df_apoe) >= 20 && length(unique(dep_df_apoe$APOE4_carrier)) >= 2) {
    ...
    export_dep_table(DEP_APOE, file.path(analysis_root, "04_sensitivity", "apoe", "AD_vs_CN_APOE_adjusted_full_limma_results_aptamer_level.csv"))
    export_dep_table(DEP_APOE_gene, file.path(analysis_root, "04_sensitivity", "apoe", "AD_vs_CN_APOE_adjusted_full_limma_results_gene_collapsed.csv"))
    safe_write_csv(apoe_compare_tbl, file.path(analysis_root, "04_sensitivity", "apoe", "primary_vs_APOE_adjusted_gene_comparison.csv"))
    safe_write_csv(summarize_dep_counts(DEP_APOE_gene, universe_label = "gene_collapsed_APOE_adjusted"), file.path(analysis_root, "04_sensitivity", "apoe", "APOE_adjusted_DEP_counts_gene_collapsed.csv"))
    export_fdr_specific_dep_tables(
      DEP_APOE_gene,
      file.path(analysis_root, "04_sensitivity", "apoe", "FDR_specific"),
...
if (RUN_FLAGS$run_cdrsb_sensitivity && !is.na(cdrsb_col)) {
  ...
  if (cdrsb_has_variation) {
    ...
    export_dep_table(DEP_CDRSB, file.path(analysis_root, "04_sensitivity", "cdrsb", "AD_only", "AD_only_CDRSB_severity_full_limma_results_aptamer_level.csv"))
    export_dep_table(DEP_CDRSB_gene, file.path(analysis_root, "04_sensitivity", "cdrsb", "AD_only", "AD_only_CDRSB_severity_full_limma_results_gene_collapsed.csv"))
    safe_write_csv(cdrsb_compare_tbl, file.path(analysis_root, "04_sensitivity", "cdrsb", "AD_only", "primary_AD_vs_CN_vs_AD_only_CDRSB_severity_alignment.csv"))
    safe_write_csv(summarize_dep_counts(DEP_CDRSB_gene, universe_label = "gene_collapsed_AD_only_CDRSB_severity"), file.path(analysis_root, "04_sensitivity", "cdrsb", "AD_only", "AD_only_CDRSB_severity_counts_gene_collapsed.csv"))
    export_fdr_specific_dep_tables(
      DEP_CDRSB_gene,
      file.path(analysis_root, "04_sensitivity", "cdrsb", "AD_only", "FDR_specific"),
...
# as a sensitivity analysis. It asks how much of the AD-vs-CN
...
if (isTRUE(RUN_FLAGS$run_cdrsb_adjusted_diagnostic_sensitivity) && !is.na(cdrsb_col)) {
  ...
  if (cdrsb_adj_has_variation) {
    ...
    export_dep_table(
      DEP_CDRSB_ADJ,
      file.path(analysis_root, "04_sensitivity", "cdrsb", "AD_vs_CN_adjusted", "AD_vs_CN_CDRSB_adjusted_full_limma_results_aptamer_level.csv")
    )
    export_dep_table(
      DEP_CDRSB_ADJ_gene,
      file.path(analysis_root, "04_sensitivity", "cdrsb", "AD_vs_CN_adjusted", "AD_vs_CN_CDRSB_adjusted_full_limma_results_gene_collapsed.csv")
    )
    safe_write_csv(
      cdrsb_adjusted_compare_tbl,
      file.path(analysis_root, "04_sensitivity", "cdrsb", "AD_vs_CN_adjusted", "primary_vs_CDRSB_adjusted_AD_vs_CN_gene_comparison.csv")
    )
    safe_write_csv(
      summarize_dep_counts(DEP_CDRSB_ADJ_gene, universe_label = "gene_collapsed_CDRSB_adjusted_AD_vs_CN"),
      file.path(analysis_root, "04_sensitivity", "cdrsb", "AD_vs_CN_adjusted", "CDRSB_adjusted_AD_vs_CN_DEP_counts_gene_collapsed.csv")
    )
    export_fdr_specific_dep_tables(
      DEP_CDRSB_ADJ_gene,
      file.path(analysis_root, "04_sensitivity", "cdrsb", "AD_vs_CN_adjusted", "FDR_specific"),
...
if (RUN_FLAGS$run_vascular_metabolic_sensitivity) {
  ...
  if (length(vascular_covariates) > 0) {
    ...
    if (nrow(dep_df_vascular) >= 20) {
      ...
      export_dep_table(DEP_VASCULAR, file.path(analysis_root, "04_sensitivity", "vascular_metabolic", "AD_vs_CN_vascular_metabolic_adjusted_full_limma_results_aptamer_level.csv"))
      export_dep_table(DEP_VASCULAR_gene, file.path(analysis_root, "04_sensitivity", "vascular_metabolic", "AD_vs_CN_vascular_metabolic_adjusted_full_limma_results_gene_collapsed.csv"))
      safe_write_csv(vascular_compare_tbl, file.path(analysis_root, "04_sensitivity", "vascular_metabolic", "main_vs_vascular_metabolic_adjusted_gene_comparison.csv"))
      safe_write_csv(summarize_dep_counts(DEP_VASCULAR_gene, universe_label = "gene_collapsed_vascular_metabolic_adjusted"), file.path(analysis_root, "04_sensitivity", "vascular_metabolic", "vascular_metabolic_adjusted_DEP_counts_gene_collapsed.csv"))
      export_fdr_specific_dep_tables(
        DEP_VASCULAR_gene,
        file.path(analysis_root, "04_sensitivity", "vascular_metabolic", "FDR_specific"),
        "AD_vs_CN_vascular_metabolic_adjusted_DEP_gene_collapsed"
      )
      safe_write_csv(tibble::tibble(covariate = vascular_covariates, n_complete_in_model = nrow(vascular_fit$metadata)), file.path(analysis_root, "04_sensitivity", "vascular_metabolic", "vascular_metabolic_covariates_used.csv"))
...
if (RUN_FLAGS$run_atn_adjusted_sensitivity) {
  ...
  if (length(preferred_atn_covariates) >= 2) {
    ...
    if (nrow(dep_df_atn) >= 20) {
      ...
      export_dep_table(DEP_ATN, file.path(analysis_root, "04_sensitivity", "atn_adjusted", "AD_vs_CN_ATN_adjusted_full_limma_results_aptamer_level.csv"))
      export_dep_table(DEP_ATN_gene, file.path(analysis_root, "04_sensitivity", "atn_adjusted", "AD_vs_CN_ATN_adjusted_full_limma_results_gene_collapsed.csv"))
      safe_write_csv(atn_compare_tbl, file.path(analysis_root, "04_sensitivity", "atn_adjusted", "primary_vs_ATN_adjusted_gene_comparison.csv"))
      safe_write_csv(summarize_dep_counts(DEP_ATN_gene, universe_label = "gene_collapsed_ATN_adjusted"), file.path(analysis_root, "04_sensitivity", "atn_adjusted", "ATN_adjusted_DEP_counts_gene_collapsed.csv"))
      safe_write_csv(tibble::tibble(covariate = preferred_atn_covariates, n_complete_in_model = nrow(atn_fit$metadata)), file.path(analysis_root, "04_sensitivity", "atn_adjusted", "ATN_covariates_used.csv"))
      export_fdr_specific_dep_tables(
        DEP_ATN_gene,
        file.path(analysis_root, "04_sensitivity", "atn_adjusted", "FDR_specific"),
...
if (RUN_FLAGS$run_secondary_clinical_severity) {
  severity_results <- run_clinical_severity_models(dep_df, dep_protein_cols, annot_tbl, file.path(analysis_root, "04_sensitivity", "clinical_severity"))
...
if (RUN_FLAGS$run_corrected_enrichment) {
  gsea_go_kegg_main <- run_gsea_go_kegg(DEP_gene, "main_dep", file.path(analysis_root, "05_enrichment_corrected", "gsea"))
  gsea_reactome_main <- run_gsea_reactome(DEP_gene, "main_dep", file.path(analysis_root, "05_enrichment_corrected", "gsea"))
  gsea_hallmark_main <- run_gsea_hallmark(DEP_gene, "main_dep", file.path(analysis_root, "05_enrichment_corrected", "gsea"))
  ora_directional_main <- run_corrected_ora_by_direction(DEP_gene, "main_dep", file.path(analysis_root, "05_enrichment_corrected", "ora"), fdr = MAIN_FDR)
  ...
  if (!is.null(DEP_APOE_gene) && nrow(DEP_APOE_gene) > 0) {
    gsea_reactome_apoe <- run_gsea_reactome(DEP_APOE_gene, "apoe_adjusted_dep", file.path(analysis_root, "05_enrichment_corrected", "gsea"))
  ...
  if (!is.null(DEP_ATN_gene) && nrow(DEP_ATN_gene) > 0) {
    gsea_reactome_atn <- run_gsea_reactome(DEP_ATN_gene, "atn_adjusted_dep", file.path(analysis_root, "05_enrichment_corrected", "gsea"))
  ...
  if (!is.null(DEP_CDRSB_gene) && nrow(DEP_CDRSB_gene) > 0) {
    gsea_reactome_cdrsb <- run_gsea_reactome(DEP_CDRSB_gene, "ad_only_cdrsb_severity", file.path(analysis_root, "05_enrichment_corrected", "gsea"))
  ...
  if (!is.null(DEP_CDRSB_ADJ_gene) && nrow(DEP_CDRSB_ADJ_gene) > 0) {
    gsea_reactome_cdrsb_adjusted <- run_gsea_reactome(DEP_CDRSB_ADJ_gene, "cdrsb_adjusted_ad_vs_cn", file.path(analysis_root, "05_enrichment_corrected", "gsea"))
...
safe_write_csv(enrichment_inventory, file.path(analysis_root, "05_enrichment_corrected", "enrichment_inventory.csv"))
...
safe_write_csv(country_counts, file.path(analysis_root, "06_robustness", "country_loco", "tables", "country_group_counts.csv"))
...
if (RUN_FLAGS$run_country_loco && length(countries_to_test) >= 2) {
  for (ctry in countries_to_test) {
    ...
    export_dep_table(dep_loco, file.path(analysis_root, "06_robustness", "country_loco", "tables", paste0("LOCO_excluding_", safe_file_tag(ctry), "_dep_results.csv")))
    cmp <- DEP %>%
    ...
    safe_write_csv(cmp, file.path(analysis_root, "06_robustness", "country_loco", "tables", paste0("LOCO_excluding_", safe_file_tag(ctry), "_comparison_to_main.csv")))
  ...
  safe_write_csv(loco_summary_metrics, file.path(analysis_root, "06_robustness", "country_loco", "tables", "LOCO_summary_metrics.csv"))
  ...
  safe_write_csv(main_vs_loco_mean, file.path(analysis_root, "06_robustness", "country_loco", "tables", "main_vs_meanLOCO_table.csv"))
}
...
if (RUN_FLAGS$run_country_meta && length(countries_to_test) >= 2) {
  ...
  safe_write_csv(country_specific_tbl, file.path(analysis_root, "06_robustness", "country_meta", "tables", "country_specific_DEP_results.csv"))
  if (nrow(country_specific_tbl) > 0) {
    ...
    safe_write_csv(meta_tbl, file.path(analysis_root, "06_robustness", "country_meta", "tables", "country_meta_analysis_results.csv"))
  }
}
...
if (RUN_FLAGS$run_country_interaction && length(countries_to_test) >= 2) {
  ...
  if (length(interaction_coef) > 0) {
    ...
    safe_write_csv(interaction_results, file.path(analysis_root, "06_robustness", "country_interaction", "tables", "country_interaction_all_terms.csv"))
    safe_write_csv(country_interaction_tbl, file.path(analysis_root, "06_robustness", "country_interaction", "tables", "country_interaction_summary_by_protein.csv"))
  }
}
...
if (RUN_FLAGS$run_site_robustness && !is.na(site_var)) {
  site_counts <- dep_df %>%
    ...
    ensure_count_cols(c("CN", "AD")) %>%
    dplyr::mutate(total = CN + AD)
  safe_write_csv(site_counts, file.path(analysis_root, "06_robustness", "site_robustness", "site_group_counts.csv"))
  ...
  if (length(sites_to_test) >= 2) {
    for (site_i in sites_to_test) {
      ...
      export_dep_table(dep_loso, file.path(analysis_root, "06_robustness", "site_robustness", "loso", "tables", paste0("LOSO_excluding_", safe_file_tag(site_i), "_dep_results.csv")))
      cmp <- DEP %>%
    ...
    safe_write_csv(loso_summary_metrics, file.path(analysis_root, "06_robustness", "site_robustness", "loso", "tables", "LOSO_summary_metrics.csv"))
  }
...
if (RUN_FLAGS$run_balanced_country_resampling && length(countries_to_test) >= 2) {
  ...
  if (is.finite(n_per_country_group) && n_per_country_group >= 3) {
    ...
    safe_write_csv(balanced_summary_tbl, file.path(analysis_root, "06_robustness", "balanced_country_resampling", "tables", "balanced_resampling_summary_metrics.csv"))
    if (nrow(balanced_all_proteins) > 0) {
      ...
      safe_write_csv(balanced_protein_tbl, file.path(analysis_root, "06_robustness", "balanced_country_resampling", "tables", "balanced_resampling_protein_stability.csv"))
    }
  }
}
...
if (RUN_FLAGS$run_formal_robustness_classification) {
  ...
  safe_write_csv(robustness_classification_tbl, file.path(analysis_root, "06_robustness", "formal_classification", "protein_robustness_classification.csv"))
  safe_write_csv(robustness_counts, file.path(analysis_root, "06_robustness", "formal_classification", "protein_robustness_classification_counts.csv"))
}
...
safe_write_csv(analysis_checkpoints, file.path(analysis_root, "07_manifest", "analysis_checkpoints.csv"))
safe_write_csv(analysis_manifest, file.path(analysis_root, "07_manifest", "analysis_manifest.csv"))
safe_write_csv(model_formula_manifest, file.path(analysis_root, "07_manifest", "model_formula_manifest.csv"))
safe_write_csv(model_design_diagnostics, file.path(analysis_root, "07_manifest", "model_diagnostics", "model_design_diagnostics.csv"))
...
safe_write_csv(tibble::tibble(saved_object = existing_workspace_objects), file.path(analysis_root, "07_manifest", "workspace_saved_objects.csv"))
if (length(missing_workspace_objects) > 0) safe_write_csv(tibble::tibble(missing_object = missing_workspace_objects), file.path(analysis_root, "07_manifest", "workspace_missing_objects.csv"))

# Canonical workspace used by downstream scripts.
save(list = existing_workspace_objects, file = file.path(analysis_root, "workspace", "analysis_workspace.RData"), envir = .GlobalEnv)

message("Primary data processing and differential analysis complete.")
message("Canonical workspace saved to: ", file.path(analysis_root, "workspace", "analysis_workspace.RData"))
message("Manifest saved to: ", file.path(analysis_root, "07_manifest", "analysis_manifest.csv"))
###############################################################################
