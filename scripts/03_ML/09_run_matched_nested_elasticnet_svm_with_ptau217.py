...
RESULTS_DIR = rf"{PROJECT_DIR}\Results_proteins_with_ptau_Matched"
...
cohort_export=df[[ID_COL,TARGET,"p.tau217"]]
cohort_export.to_csv(rf"{RESULTS_DIR}\cohort_common.csv",index=False)
...
for fold,(train_idx,test_idx) in enumerate(
    ...
    X_train=(X_ptau.iloc[train_idx].copy())
    X_test=(X_ptau.iloc[test_idx].copy())
    ...
    search.fit(X_train,y_outer_train)
    best_params_ptau.append(search.best_params_)   
    y_prob=search.best_estimator_.decision_function(X_test)
    oof_ptau[test_idx]=(y_prob-y_prob.mean())/(y_prob.std()+1e-8)
    y_pred=(y_prob>=0).astype(int)
    tn,fp,fn,tp=confusion_matrix(y_outer_test,y_pred).ravel()
    ...
    X_ptau.iloc[train_idx].reset_index(drop=True)
    ],axis=1)

    X_test_combo=pd.concat([
    X_outer_test[panel].reset_index(drop=True),
    X_ptau.iloc[test_idx].reset_index(drop=True)
    ],axis=1)
...
metrics_ptau.to_csv(rf"{RESULTS_DIR}\metrics_ptau.csv",index=False)
...
metrics_ptau.mean(numeric_only=True).to_frame().T.assign(Model="pTau217"),
metrics_combo.mean(numeric_only=True).to_frame().T.assign(Model="Combo")
])

pd.DataFrame(selection_summary_soma).to_csv(rf"{RESULTS_DIR}\feature_counts.csv",index=False)
...
print("\nOOF PTAU217")
print(roc_auc_score(y,oof_ptau))
print("\nOOF COMBO")
