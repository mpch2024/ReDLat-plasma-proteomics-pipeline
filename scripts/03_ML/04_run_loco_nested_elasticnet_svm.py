...
RESULTS_DIR = rf"{PROJECT_DIR}\Results_LOCO"
...
df["site"]=df.index.str[:2]
df["country"]=df["site"].map(site_to_country)
...
for fold,(train_idx,test_idx) in enumerate(
    ...
    country=df.iloc[test_idx]["country"].iloc[0]
    ...
    "Country":country,"Fold":fold,"Model":"Proteins","FPR":fpr,"TPR":tpr,"Threshold":thr,"AUC":auc}))    
...
panel_final=(panel_freq[panel_freq>=0.8].index.tolist())
...
# =====================================================
# COUNTRY PERMUTATION EXPORT
# =====================================================

perm_country_df = pd.concat(perm_country,ignore_index=True)
perm_country_df = (perm_country_df.sort_values(
["Country","Permutation_mean"],ascending=[True,False]))

perm_country_df.to_csv(rf"{RESULTS_DIR}\permutation_importance_country.csv",index=False)

print("\nCountry permutation")

print(perm_country_df.head())
